export default {
  async fetch(request, env) {
    ...
    // === 3. Notification d'intention de paiement (Wero / ANCV) ===
    // Ces moyens de paiement ne passent pas par Stripe : on ne peut pas
    // vérifier automatiquement que l'argent a bien été reçu. On envoie donc
    // simplement un email d'alerte au propriétaire dès que la cliente clique,
    // pour qu'il puisse suivre le dossier et bloquer le calendrier à temps.
    if (url.pathname === '/notify-intent') {
      ...
      const { mode, tente, arrivee, depart, nuits, montant, prenom, nom, email, tel, message } = body;

      if (!mode || !tente || !arrivee || !depart) {
        return new Response('Paramètres manquants', { status: 400 });
      }

      const modeLabel = mode === 'wero' ? 'Wero' : mode === 'ancv' ? 'Chèques-Vacances ANCV' : mode === 'whatsapp' ? 'WhatsApp / Autre moyen' : mode;

      try {
        await sendEmail(env, {
          to: 'contact@glamping-keroman.fr',
          subject: `⚠ Intention de paiement ${modeLabel} — Tente ${tente} (${arrivee})`,
          html: `
            <div style="font-family:sans-serif;max-width:560px;margin:0 auto">
              <h2 style="color:#d97706">Nouvelle intention de réservation — ${modeLabel}</h2>
              <p style="background:#fef3c7;padding:10px 14px;border-radius:8px;color:#92400e">
                ⚠ Ce paiement n'est <strong>pas automatiquement vérifié</strong> (contrairement à Stripe).
                Vérifiez vous-même la réception avant de confirmer la réservation.
              </p>
              <table style="width:100%;border-collapse:collapse;margin:20px 0">
                <tr><td style="padding:8px 0;color:#57534e">Client</td><td style="padding:8px 0;font-weight:bold">${prenom || ''} ${nom || ''}</td></tr>
                <tr><td style="padding:8px 0;color:#57534e">Email</td><td style="padding:8px 0;font-weight:bold">${email || 'Non fourni'}</td></tr>
                <tr><td style="padding:8px 0;color:#57534e">Téléphone</td><td style="padding:8px 0;font-weight:bold">${tel || 'Non fourni'}</td></tr>
                ...
                <tr><td style="padding:8px 0;color:#57534e">Montant attendu</td><td style="padding:8px 0;font-weight:bold">${montant || '?'} €</td></tr>
                <tr><td style="padding:8px 0;color:#57534e">Moyen de paiement</td><td style="padding:8px 0;font-weight:bold">${modeLabel}</td></tr>
                ${message ? `<tr><td style="padding:8px 0;color:#57534e">Message</td><td style="padding:8px 0">${message}</td></tr>` : ''}
              </table>
              <p style="color:#d97706">⚠ Pensez à vérifier la réception du paiement et à bloquer ces dates sur le calendrier Google si vous confirmez la réservation.</p>
            </div>
          `
        });
      } catch (e) {
        console.error('Erreur envoi email intention:', e.message);
        // On ne fait pas échouer la requête pour ne pas bloquer le parcours client.
      }
    ...
    // === 4. Webhook Stripe : envoi automatique des emails de confirmation ===
